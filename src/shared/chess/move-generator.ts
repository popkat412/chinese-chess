...
function blockablePieceGenerator(
  position: Pair,
  board: PieceGrid,
  dx: number[],
  dy: number[],
  ...
  console.assert(
    multipleEquals(dx.length, dy.length, blockX.length, blockY.length)
  );
...
function kingBoxPieceGenerator(
  position: Pair,
  board: PieceGrid,
  dx: number[],
  dy: number[]
...
function isInsideKingBox(toCheck: Pair, side: PieceSide): boolean {
  ...
  if (side == PieceSide.Red) {
    // Red is always at top of the grid
    return x <= 2 && x >= 0 && y <= 5 && y >= 3;
  } else {
    // Black is always at the bottom
...
function pawnGenerator(position: Pair, board: PieceGrid): Move[] {
  ...
  if (isAcrossRiver(position, piece.side)) {
    ...
    if (board[nx][ny]?.side == piece.side) {
      return [];
    } else {
